...
// Okno czasowe, z którego liczymy średnią odchylenia w centach.
// Więcej próbek w oknie = mniej nerwowe reakcje na jeden słaby odczyt.
const RECENT_WINDOW_MS = 900;

// Ile ms nowy kandydat na kolor musi się utrzymać, zanim faktycznie
// zostanie namalowany — zapobiega miganiu tuż po uśrednieniu.
const COLOR_HOLD_MS = 110;

// Progi tolerancji (mniej wymagające niż poprzednio: 30/45).
const IN_TUNE_CENTS = 40;
const SLIGHT_OFF_CENTS = 60;

// Jaki ułamek próbek w oknie musi się zgadzać z docelową nutą,
// żeby liczyć kolor na podstawie średniej, a nie od razu uznać
// to za "śpiewasz całkiem inną nutę".
...
/**
 * Decyduje o kolorze nuty na podstawie UŚREDNIONYCH odczytów
 * (colorInputs), a nie pojedynczej, najnowszej klatki mikrofonu.
 *
 * colorInputs pochodzi z getColorInputsForNote():
 * - hasSignal: czy w oknie czasowym był jakikolwiek odczyt
 * - totalCount: liczba wszystkich odczytów w oknie
 * - matchedCount: liczba odczytów zgodnych z docelową nutą
 * - matchRatio: matchedCount / totalCount
 * - averagedCents: średnie odchylenie (tylko z odczytów zgodnych)
 */
function pickNoteColor(isActive, colorInputs, targetNoteName) {
    ...
    // Zbyt mało próbek zgadza się z docelową nutą — traktujemy to
    // jako "śpiewasz coś innego", niezależnie od tego, jak bliskie
    // w centach byłyby te nieliczne zgodne próbki.
...
export default function NoteHighway({ musicXmlUrl, audioRef, liveNote, liveCents, liveClarity, notes: notesProp }) {
    ...
    useEffect(() => {
        ...
        // Bufor ostatnich odczytów mikrofonu (okno RECENT_WINDOW_MS),
        // resetowany zawsze, gdy ten efekt się przemontowuje.
        const recentReadingsRef = [];

        // Pamięć koloru per-nuta, dla mechanizmu "hold" (COLOR_HOLD_MS).
    ...
    if (recentReadingsRef.length === 0) {
        return { hasSignal: false, totalCount: 0, matchedCount: 0, matchRatio: 0, averagedCents: null };
    }

    const totalCount = recentReadingsRef.length;
    const targetChroma = stripOctave(targetNoteName);

    const matching = recentReadingsRef.filter(
        (reading) => stripOctave(reading.note) === targetChroma && Number.isFinite(reading.cents)
    );
    ...
    return { hasSignal: true, totalCount, matchedCount, matchRatio, averagedCents };
        ...
        function draw() {
            ...
            // Jeden odczyt mikrofonu na klatkę rysowania — dotyczy
            // bieżącego dźwięku, nie konkretnej nuty na highway.
            ...
            for (let i = 0; i < notes.length; i++) {
                ...
                if (isActive) {
    console.log(
        `target=${targetNoteName} live=${liveNoteRef.current} cents=${liveCentsRef.current?.toFixed?.(1)} ` +
        `matched=${colorInputs.matchedCount}/${colorInputs.totalCount} ratio=${colorInputs.matchRatio.toFixed(2)} ` +
        `avgCents=${colorInputs.averagedCents}`
    );
}
