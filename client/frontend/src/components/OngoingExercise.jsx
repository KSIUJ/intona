...
/*
 * No artificial upper cap here: deviation is measured in cents, which
 * naturally exceeds 100 for an off-pitch note. We only guard against
 * non-finite values and negative numbers. If the backend column
 * genuinely requires a hard ceiling, use something musically
 * meaningful like 1200 (one octave) instead of 100.
 */
...
export default function OngoingExercise() {
    ...
    /*
     * Raw results.json entries used by Gaussian scoring.
     */
    ...
    /*
     * Every microphone frame captured while the backing track is playing.
     */
    ...
    /*
     * Receives every frame from Pitchy (throttled inside usePitchDetection
     * only for the React state used for display — this callback still
     * gets called for scoring on every raw analysed frame).
     */
    const handlePitchFrame = useCallback(
        (frame) => {
            ...
            /*
             * ~12 frames per second is enough for scoring and avoids
            ...
            /*
             * Live deviation is calculated against the CURRENT TARGET NOTE,
             * not against the nearest chromatic note. Octave differences
             * are normalized away, matching the backend Gaussian scoring.
             */
            const activeTargetNote = findActiveTargetNote(
            ...
            if (
                ...
                const targetFrequency = Number(activeTargetNote.frequency_hz);

                const deviation = calculateCentsDeviation(
                    normalizeToNearestOctave(frame.frequency, targetFrequency),
                    targetFrequency
                );

                if (deviation !== null && Number.isFinite(deviation)) {
                    const meterDeviation = Math.max(
                        -50,
                        Math.min(50, Math.round(deviation))
                    );
            ...
            /*
             * For LIVE scoring, exclude notes that have not started yet.
             */
            ...
            const liveDeviation = clampDeviationForBackend(
                liveResult.averageDeviation ?? 0
            );

            liveScoreRef.current = liveScore;
            liveDeviationRef.current = liveDeviation;

            setTimeInTune(liveScore);
            setAverageDeviation(liveDeviation);
        },
        [audio.audioRef]
    );
    ...
    const communicateExerciseEnd = async (
        ...
        const apiResponse = await fetch(`/api/exercises/${logId}/end`, {
            ...
            /*
             * Existing backend field time_in_tune stores our final
             * 0-100 Gaussian score.
             */
            body: JSON.stringify({
    ...
    const handleEndOfExercise = (
        ...
        /*
         * Completed song: score ALL target notes.
         * Manually stopped exercise: score only notes that had already started.
         */
    ...
    /*
     * Microphone starts once when OngoingExercise opens.
     * Frames are only recorded while the backing track is actually playing.
     */
    useEffect(() => {
        startMicrophone();
    ...
    useEffect(() => {
        ...
        if (exerciseAccessToken !== null) {
            ...
            const storedDeviation = parseFloat(
