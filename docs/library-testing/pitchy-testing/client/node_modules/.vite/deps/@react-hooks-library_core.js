...
//#region node_modules/@react-hooks-library/core/index.esm.js
...
function useMediaQuery(query) {
	...
	(0, import_react.useEffect)(() => {
		...
		"addEventListener" in mediaQuery ? mediaQuery.addEventListener("change", handler) : mediaQuery.addListener(handler);
		return () => {
			"addEventListener" in mediaQuery ? mediaQuery.removeEventListener("change", handler) : mediaQuery.removeListener(handler);
		};
	}, [query]);
	return matches;
}
...
function BreakPointHooks(breakpoints) {
	return {
		...
		**/
		useBetween: (a, b) => {
			return useMediaQuery(`(min-width: ${toWidth(breakpoints[a])}) and (max-width: ${toWidth(breakpoints[b])})`);
		},
		/**
		...
		**/
		isInBetween(a, b) {
			return match(`(min-width: ${toWidth(breakpoints[a])}) and (max-width: ${toWidth(breakpoints[b])})`);
		}
	};
}
function useEventListener(...args) {
	...
	const savedListener = (0, import_react.useRef)(listener);
	const cleanup = (0, import_react.useRef)(noop);
	(0, import_react.useEffect)(() => {
...
function useActiveElement() {
	const [activeElement, setActiveElement] = (0, import_react.useState)(() => _document === null || _document === void 0 ? void 0 : _document.activeElement);
...
function useAsyncCallback(callback) {
	...
	const _callback = (0, import_react.useCallback)(async (...args) => {
		try {
			setIsLoading(true);
...
function useClickOutside(target, handler, options = {}) {
	...
	return useEventListener(_window, event, (0, import_react.useCallback)((event) => {
		const el = unRef(target);
		if (!el) return;
		if (el === event.target || event.composedPath().includes(el)) return;
		handler(event);
	}, [handler, target]), { passive: true });
}
/**
...
function useFont(family, source, descriptors) {
	...
	(0, import_react.useEffect)(() => {
		...
		setFont(font);
		setLoaded(false);
		font.load().then(() => document.fonts.add(font)).catch(() => setError(true)).finally(() => setLoaded(true));
	}, [
		descriptors,
		family,
		source
...
function useIntersectionObserver(target, options = {}, callback = noop) {
	...
	(0, import_react.useEffect)(() => {
		...
		(_a = observer.current) === null || _a === void 0 || _a.observe(el);
		return stop;
	}, [
		callback,
		isSupported,
...
function useInterval(callback, delay, options) {
	...
	const tickId = (0, import_react.useRef)(void 0);
	(0, import_react.useEffect)(() => {
		savedCallback.current = callback;
		if (!paused && immediate) callback();
	}, [
		callback,
		immediate,
		paused
...
function useKeyStroke(keys, handler, options = {}) {
	...
	return useEventListener(target, eventName, (0, import_react.useCallback)((e) => {
		const eventKey = code ? e.code : e.key;
		if (typeof keys === "string" ? keys === eventKey : keys.includes(eventKey)) handler(e);
	}, [
		code,
		handler,
		keys
...
function useLocalStorage(key, initialValue, options) {
	...
	return [storedValue, (0, import_react.useCallback)((value) => {
		try {
...
function useLocation() {
	...
	useEventListener("popstate", () => setState(buildState("popstate")), { passive: true });
	useEventListener("hashchange", () => setState(buildState("hashchange")), { passive: true });
	return state;
}
/**
...
function useMediaStream(options = {}) {
	...
	const isSupported = useIsSupported(() => {
		var _a;
		return !!((_a = _navigator === null || _navigator === void 0 ? void 0 : _navigator.mediaDevices) === null || _a === void 0 ? void 0 : _a.getUserMedia);
	});
	const stream = (0, import_react.useRef)(null);
	const ref = (0, import_react.useRef)(null);
	const [isPlaying, setPlaying] = (0, import_react.useState)(false);
	const [isAudioMuted, setAudioMuted] = (0, import_react.useState)(false);
	const [isVideoMuted, setVideoMuted] = (0, import_react.useState)(false);
	const getDeviceOptions = (0, import_react.useCallback)((device) => {
	...
	const play = (0, import_react.useCallback)(async () => {
		...
		stream.current = (_a = await (_navigator === null || _navigator === void 0 ? void 0 : _navigator.mediaDevices.getUserMedia({
	...
	const stop = (0, import_react.useCallback)(() => {
		var _a;
		setPlaying(false);
		(_a = stream.current) === null || _a === void 0 || _a.getTracks().forEach((t) => t.stop());
		stream.current = null;
	}, []);
	...
	const muteAudio = (0, import_react.useCallback)(() => {
		var _a;
		setAudioMuted(true);
		(_a = stream.current) === null || _a === void 0 || _a.getAudioTracks().forEach((t) => t.enabled = false);
	}, []);
	const unMuteAudio = (0, import_react.useCallback)(() => {
		var _a;
		setAudioMuted(false);
		(_a = stream.current) === null || _a === void 0 || _a.getAudioTracks().forEach((t) => t.enabled = true);
	}, []);
	const muteVideo = (0, import_react.useCallback)(() => {
		var _a;
		setVideoMuted(true);
		(_a = stream.current) === null || _a === void 0 || _a.getVideoTracks().forEach((t) => t.enabled = false);
	}, []);
	const unMuteVideo = (0, import_react.useCallback)(() => {
		var _a;
		setVideoMuted(false);
		(_a = stream.current) === null || _a === void 0 || _a.getVideoTracks().forEach((t) => t.enabled = true);
	}, []);
...
function useMouse(options = {}) {
	const { touch = true, type = "client", resetOnTouchEnds = false, initialValue = {
		x: 0,
		y: 0
	} } = options;
	const [x, setX] = (0, import_react.useState)(initialValue.x);
	const [y, setY] = (0, import_react.useState)(initialValue.y);
	...
	(0, import_react.useEffect)(() => {
		...
		if (touch) {
			...
			if (resetOnTouchEnds) window.addEventListener("touchend", reset, { passive: true });
		}
		return () => {
...
function useMutationObserver(target, callback, options = {}) {
	...
	(0, import_react.useEffect)(() => {
		...
		(_a = observer.current) === null || _a === void 0 || _a.observe(el, options);
		return stop;
	}, [
		callback,
		stop,
...
function useNetwork() {
	...
	const [offlineAt, setOfflineAt] = (0, import_react.useState)(void 0);
	const connection = (0, import_react.useRef)(void 0);
	const rerender = (0, import_react.useState)({})[1];
	useMount(() => {
		if (!_navigator) return;
		setIsOnline(_navigator.onLine);
		setOfflineAt(isOnline ? void 0 : Date.now());
		const _connection = _navigator.connection;
...
function useScreenShare(options = {}) {
	...
	const isSupported = useIsSupported(() => {
		var _a;
		return !!((_a = _navigator === null || _navigator === void 0 ? void 0 : _navigator.mediaDevices) === null || _a === void 0 ? void 0 : _a.getDisplayMedia);
	});
	const stream = (0, import_react.useRef)(null);
	const ref = (0, import_react.useRef)(null);
	const [isPlaying, setPlaying] = (0, import_react.useState)(false);
	const play = (0, import_react.useCallback)(async () => {
		...
		stream.current = (_a = await (_navigator === null || _navigator === void 0 ? void 0 : _navigator.mediaDevices.getDisplayMedia({
			audio,
			video
		}))) !== null && _a !== void 0 ? _a : null;
		setPlaying(true);
	...
	(0, import_react.useEffect)(() => {
		...
		if (!ref.current) return;
		ref.current.srcObject = stream.current;
		(_a = stream.current) === null || _a === void 0 || _a.getVideoTracks()[0].addEventListener("ended", stop);
	}, [isPlaying, stop]);
	return {
		isSupported,
		isPlaying,
...
function useScrollIntoView(target, options = {}) {
	...
	useMount(() => {
		const el = unRef(target);
		if (!(el && (isFunction(predicate) ? predicate() : predicate))) return;
...
function useStateCompare({ initialValue, compare }) {
	const [state, _setState] = (0, import_react.useState)(initialValue);
	return [state, (0, import_react.useCallback)((value) => {
		typeof value === "function" ? _setState(value) : _setState((oldValue) => compare(oldValue, value));
	}, [compare])];
}
/**
*
...
function useStateHistory(defaultValue, options = {}) {
	...
	const lastSaved = (0, import_react.useRef)(isFunction(defaultValue) ? defaultValue() : defaultValue);
	const rerender = (0, import_react.useState)({})[1];
	const actionHistory = (0, import_react.useRef)([]);
	const redoHistory = (0, import_react.useRef)([]);
	const redoAllowed = (0, import_react.useRef)(false);
	const push = (0, import_react.useCallback)((value) => {
		if (actionHistory.current.length < maxHistory) actionHistory.current.push(value);
		else {
	...
	return {
		...
		reset: (0, import_react.useCallback)(() => {
			var _a;
			if (!((_a = actionHistory.current) === null || _a === void 0 ? void 0 : _a.length)) return;
...
function useTitle(newTitle) {
	const [title, setTitle] = (0, import_react.useState)(newTitle !== null && newTitle !== void 0 ? newTitle : "");
	useMount(() => {
		var _a;
...
function useToggle(defaultValue = false) {
	...
	return {
		bool,
		toggle: (0, import_react.useCallback)(() => setBool((s) => !s), []),
		setTrue: (0, import_react.useCallback)(() => setBool(true), []),
		setFalse: (0, import_react.useCallback)(() => setBool(false), [])
	};
}
/**
...
//#endregion
export { BreakPointHooks, breakpointsAntDesign, breakpointsBootstrapV5, breakpointsSematic, breakpointsTailwind, breakpointsVuetify, useActiveElement, useAsyncCallback, useClickOutside, useDebounce, useEventListener, useFont, useHasMounted, useHover, useIntersectionObserver, useInterval, useIsSupported, useKeyDown, useKeyPressed, useKeyStroke, useKeyUp, useLocalStorage, useLocation, useMediaQuery, useMediaStream, useMouse, useMutationObserver, useNetwork, useOnline, usePreferredColorScheme, usePrevious, useScreenShare, useScroll, useScrollIntoView, useSessionStorage, useStateCompare, useStateHistory, useTitle, useToggle, useWindowSize };
